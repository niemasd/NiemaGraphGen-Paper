...
```bash
# NiemaGraphGen
for n in 100 1000 10000 100000 1000000 ; do for ed in X ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.complete.ngg.r$r.txt ~/NiemaGraphGen-Paper/tools/ngg_1.0.0/ngg_complete $n > /dev/null ; done ; done ; done

# NetworkX
for n in 100 1000 10000 100000 ; do for ed in X ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.complete.nx.r$r.txt ~/NiemaGraphGen-Paper/tools/nx_2.5.1/nx_complete.py $n > /dev/null ; done ; done ; done

# iGraph
for n in 100 1000 10000 100000 1000000 ; do for ed in X ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.complete.ig.r$r.txt ~/NiemaGraphGen-Paper/tools/ig_0.9.4/ig_complete.py $n > /dev/null ; done ; done ; done
```
