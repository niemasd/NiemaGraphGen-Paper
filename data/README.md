...
# NetworkX
for n in 100 1000 10000 100000 ; do for ed in 10 20 40 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.er.nx.r$r.txt ~/NiemaGraphGen-Paper/tools/nx_2.5.1/nx_erdos_renyi.py $n $(echo "$ed / $n" | bc -l) > /dev/null ; done ; done ; done
...
### Expected Degree = 10 → Lattice Degree = 10 - 2 = 8 → *p* = 0.25
```bash
# NiemaGraphGen
for n in 100 1000 10000 100000 1000000 ; do for ed in 10 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.nws.ngg.r$r.txt ~/NiemaGraphGen-Paper/tools/ngg_1.0.0/ngg_newman_watts_strogatz $n $(echo "$ed - 2" | bc -l) 0.25 > /dev/null ; done ; done ; done

# NetworkX
for n in 100 1000 10000 100000 ; do for ed in 10 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.nws.nx.r$r.txt ~/NiemaGraphGen-Paper/tools/nx_2.5.1/nx_newman_watts_strogatz.py $n $(echo "$ed - 2" | bc -l) 0.25 > /dev/null ; done ; done ; done
```

### Expected Degree = 20 → Lattice Degree = 20 - 2 = 18 → *p* = 0.1115
```bash
# NiemaGraphGen
for n in 100 1000 10000 100000 1000000 ; do for ed in 20 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.nws.ngg.r$r.txt ~/NiemaGraphGen-Paper/tools/ngg_1.0.0/ngg_newman_watts_strogatz $n $(echo "$ed - 2" | bc -l) 0.1115 > /dev/null ; done ; done ; done
...
```bash
# NiemaGraphGen
for n in 100 1000 10000 100000 1000000 ; do for ed in 10 20 40 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.complete.ngg.r$r.txt ~/NiemaGraphGen-Paper/tools/ngg_1.0.0/ngg_complete $n > /dev/null ; done ; done ; done

# NetworkX
for n in 100 1000 10000 100000 ; do for ed in 10 20 40 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.complete.nx.r$r.txt ~/NiemaGraphGen-Paper/tools/nx_2.5.1/nx_complete.py $n > /dev/null ; done ; done ; done

# iGraph
for n in 100 1000 10000 100000 1000000 ; do for ed in 10 20 40 ; do for r in $(seq -w 1 10); do /usr/bin/time -v -o n$n/ed$ed/time.complete.ig.r$r.txt ~/NiemaGraphGen-Paper/tools/ig_0.9.4/ig_complete.py $n > /dev/null ; done ; done ; done
```
